...
- Crypto boundary is enforced: all keypair generation and primitive sign/verify/hash live in the sibling `keylock` crate. authbox depends on `keylock::generate_ed25519_keypair`/`generate_ecdsa_p256/384/521_keypair`/`generate_ecdsa_secp256k1_keypair`/`generate_ed448_keypair`/`generate_rsa_keypair_with_bits`/`generate_x25519_keypair`/`keccak256` plus the `keylock::crypto::*` and `keylock::hash::*` primitives. `authbox::pki::KeyPair` is a re-export of `keylock::crypto::KeyPair`; `authbox::pki::generate_*_keypair`, `pki::ed25519`, `pki::ed448`, `pki::ecdsa`, and `pki::hash` are thin shims to keylock. PKI-format-specific helpers stay in authbox: SPKI encode/decode for every supported curve, SEC1 encode/decode for P-256/P-384/P-521, PKCS#8 DER + PEM `ENCRYPTED PRIVATE KEY` wrappers, RSA PKCS#1/PKCS#8 codec, and the X25519+XSalsa20 key-exchange envelope.
